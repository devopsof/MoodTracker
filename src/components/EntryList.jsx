...
import { deleteEntryFromLocal } from '../utils/localStorageApi'
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="p-2 rounded-lg text-white/60 hover:text-red-400 hover:bg-white/10 transition-all duration-200 opacity-0 group-hover:opacity-100"
                        title="Delete entry"
                      >
                      ...
                      {showDeleteConfirm && entryToDelete && entryToDelete.id === entry.id && (
                        <div className="absolute top-full right-0 mt-2 z-50 bg-gray-900 border border-white/20 rounded-xl p-4 shadow-xl w-64">
                          <p className="text-white text-sm mb-3">Are you sure you want to delete this entry?</p>
                          <div className="flex justify-end gap-2">
                            <button
                              ...
                              className="px-3 py-1 text-xs rounded-lg bg-white/10 text-white hover:bg-white/20 transition-all duration-200"
                            >
                              Cancel
                            </button>
                            <button
                              onClick={async () => {
                                try {
                                  ...
                                  // Get user email from the current context
                                  const userEmail = localStorage.getItem('currentUserEmail');
                                  
                                  // Delete from API if online
                                  if (window.navigator.onLine) {
                                    try {
                                      await deleteEntry(entryToDelete.id, userEmail);
                                      console.log('✅ Entry deleted from API:', entryToDelete.id);
                                    } catch (apiError) {
                                      console.error('❌ API delete failed, falling back to local:', apiError);
                                      // Fall back to local delete if API fails
                                      deleteEntryFromLocal(userEmail, entryToDelete.id);
                                    }
                                  } else {
                                    // Offline mode - delete from localStorage only
                                    deleteEntryFromLocal(userEmail, entryToDelete.id);
                                    console.log('📱 Offline delete from localStorage:', entryToDelete.id);
                                  }
                                  
                                  // Update UI through parent component
                                  if (onEntryUpdate) {
                                    onEntryUpdate(null, entryToDelete.id);
                                  }
                              ...
                              className="px-3 py-1 text-xs rounded-lg bg-red-600 text-white hover:bg-red-700 transition-all duration-200"
                            >
                              Yes, Delete
                            </button>
                          </div>
